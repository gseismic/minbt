...
- 目标等于当前状态时返回 `Order(status="skipped", side="none", qty=0, reason="target unchanged")`。
...
- 没有持仓时返回 `Order(status="skipped", side="none", qty=0, reason="position empty")`。
- 缺少价格时抛异常。
...
|目标仓位无变化|`Order(status="skipped", qty=0, reason="target unchanged")`|
|空仓平仓|`Order(status="skipped", qty=0, reason="position empty")`|
