...
def test_position_open_new():
    ...
    # 测试开空仓
    pos_short = Position(symbol="BTC-USDT", leverage=2.0)
    released_margin, realized_pnl = pos_short.commit_open_new(price=50000.0, qty=-1.0)
    assert pos_short.side == "short"
    
    # 异常情况 - 反向开仓
    with pytest.raises(AssertionError):
        pos.commit_open_new(price=50000.0, qty=-1.0)  # 多仓不能开空
    with pytest.raises(AssertionError):
        pos_short.commit_open_new(price=50000.0, qty=1.0)  # 空仓不能开多
...
def test_position_close_all():
    """测试全部平仓"""
    pos = Position(symbol="BTC-USDT", leverage=2.0)
    released_margin, realized_pnl = pos.commit_open_new(price=50000.0, qty=2.0)
